addLayer("ex", {
	...
	buyables: {
		...
		12: {
			cost() {return new Decimal(3).pow(getBuyableAmount(this.layer, this.id).add(1))},
			effectBase() {
				let base = getBuyableAmount(this.layer, this.id);
				base = base.mul(buyableEffect("ex", 21));
				return base;
		...
		21: {
			cost() {return new Decimal(10).pow(getBuyableAmount(this.layer, this.id).add(1).pow(2))},
			effectBase() {return 2},
			effect() {return new Decimal(this.effectBase()).pow(getBuyableAmount(this.layer, this.id))},
			title: "influence tickspeed",
			display() {return "all generators produce " + format(this.effectBase()) + "x as much<br><br>Effect: " + format(this.effect()) + "x<br><br>Cost: " + formatWhole(this.cost()) + " influence<br><br>Bought: " + formatWhole(getBuyableAmount(this.layer, this.id))},
