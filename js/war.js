...
const warUpgrades = [[
	...
	{title: "Wilderness Exploration", desc() {return "divides species requirement based on wars<br>(currently /" + format(gridEffect("w", 103)) + ")"}, effect() {return player.w.points.pow_base(1e5)}, cost: 4},
	...
	{title: "Forced Migration", desc() {return "increases the 10th hybridization's completion limit based on wars<br>(currently +" + formatWhole(gridEffect("w", 201)) + ")"}, effect() {return player.w.points.mul(2)}, cost: 2, e: {effect() {return player.w.points.mul(2)}, cost: 1}},
...
addLayer("w", {
	...
	grid: {
		...
		onClick(data, id) {
			if (data == 1) player.w.spentE = player.w.spentE.add(warUpgrades[Math.floor(id / 100) - 1][id % 100 - 1].e.cost);
		...
		getDisplay(data, id) {
			...
			return (typeof desc == "function" ? desc() : desc) + "<br><br>Cost: " + (data > 0 && Math.floor(id / 100) <= getEnhancableGridSize() && id % 100 <= getEnhancableGridSize() ? formatWhole(upg.e?.cost || Infinity) + " battle enhancements" : formatWhole(upg.cost || Infinity) + " battles");
		},
		getEffect(data, id) {
			if (!id) return;
			const upg = warUpgrades[Math.floor(id / 100) - 1][id % 100 - 1];
